    ...
    methods: {
      async getCode() {
        if(!this.isTimeOn){
          let timeOn = false;
          let m = 30
          this.placeholder = '已发送(' + m +')'
          this.isTimeOn = true
          const timeID = setInterval(()=>{
            timeOn = true;
            if(m <= 0){
              this.sendCodeFaild(timeID)
            }else{
              this.placeholder = '已发送(' + --m +'s)'
            }
          }, 1000)
      ...
      sendCodeFaild(timeID){  //验证码发送失败或过时时调用
        this.placeholder = '重新发送验证码'
        this.isTimeOn = false
